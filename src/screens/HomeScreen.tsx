/**
 * Global Quest - Premium Home Screen
 * With entrance animations and navigation
 */

import React, { useEffect, useRef } from 'react';
import { StyleSheet, View, Text, ScrollView, TouchableOpacity, Dimensions, Animated, Easing, Platform } from 'react-native';
...
const usePulseAnimation = (delay: number = 0) => {
    ...
    useEffect(() => {
        Animated.sequence([
            ...
            Animated.loop(
                Animated.sequence([
                    Animated.timing(scale, { toValue: 1.05, duration: 1500, easing: Easing.inOut(Easing.ease), useNativeDriver: true }),
...
const useFireAnimation = () => {
    ...
    useEffect(() => {
        fires.forEach((fire, index) => {
            Animated.loop(
                Animated.sequence([
                    Animated.timing(fire, { toValue: 1, duration: 300 + index * 50, easing: Easing.inOut(Easing.ease), useNativeDriver: true }),
                    Animated.timing(fire, { toValue: 0, duration: 300 + index * 50, easing: Easing.inOut(Easing.ease), useNativeDriver: true }),
                ])
            ).start();
        });
    }, []);
...
const StarryBackground = () => (
    <View style={styles.bgContainer}>
        <LinearGradient colors={['#030712', '#0a1628', '#111d32', '#0a1628']} locations={[0, 0.3, 0.7, 1]} style={StyleSheet.absoluteFill} />
        ...
        {Array.from({ length: 30 }).map((_, i) => (
            <AnimatedStar key={i} delay={Math.random() * 2000} left={`${Math.random() * 100}%`} top={`${Math.random() * 100}%`} size={Math.random() * 2 + 1} />
        ))}
    </View>
);
    ...
    return (
        <Animated.View style={[styles.header, { opacity, transform: [{ translateY }] }]}>
            ...
            <View style={styles.headerRight}>
                ...
                <TouchableOpacity style={styles.headerIconPrimary} onPress={() => navigation.navigate('Profile' as never)}>
                    <ProfileIcon />
                </TouchableOpacity>
            </View>
        </Animated.View>
...
const DailyChallengeCard = () => {
    ...
    return (
        <Animated.View style={{ opacity, transform: [{ translateY }, { scale }] }}>
            <TouchableOpacity activeOpacity={0.95} style={styles.dailyWrapper} onPress={() => navigation.navigate('Question' as never)}>
                <LinearGradient colors={colors.gradients.dailyChallenge} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.dailyCard}>
                    ...
                    <Animated.View style={{ transform: [{ scale: pulseScale }] }}>
                        <TouchableOpacity style={styles.dailyButton} onPress={() => navigation.navigate('Question' as never)}>
            ...
            <View style={styles.streakWrapper}>
                <LinearGradient colors={[colors.bg.card, colors.bg.cardLight]} style={styles.streakCard}>
                    ...
                    <View style={styles.fireRow}>
                        {fires.map((fire, i) => (
                            <Animated.Text key={i} style={[styles[`fire${i + 1}` as keyof typeof styles], { transform: [{ translateY: fire.interpolate({ inputRange: [0, 1], outputRange: [0, -5] }) }, { scale: fire.interpolate({ inputRange: [0, 1], outputRange: [1, 1.1] }) }] }]}>🔥</Animated.Text>
                        ))}
                    </View>
    ...
    return (
        <Animated.View style={{ opacity, transform: [{ scale }] }}>
            <TouchableOpacity style={styles.continentItem}>
                ...
                {continent.progress >= 80 && <View style={styles.completeBadge}><Text style={styles.completeText}>Complete</Text></View>}
            </TouchableOpacity>
        </Animated.View>
    );
};
            ...
            <View style={styles.exploreWrapper}>
                <LinearGradient colors={[colors.bg.card, colors.bg.cardLight]} style={styles.exploreCard}>
                    ...
                    <View style={styles.continentGrid}>
                        {mockData.continents.map((continent, index) => <ContinentItem key={continent.id} continent={continent} index={index} />)}
                    </View>
